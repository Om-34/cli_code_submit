...
export const saveProblem =
  ...
  async (prob, dir) => {
    ...
    fs.writeFileSync(path.join(dir, "CodeMain.java"), prob.boiler);
...
export const compressProblem =
  ...
  (dir) => {
    ...
    prob.boiler = fs.readFileSync(path.join(dir, "CodeMain.java")).toString();
...
export const compressSolution =
  ...
  (dir) => {
    ...
    solu.code = fs.readFileSync(path.join(dir, "CodeMain.java")).toString();
    return solu;
  };
export const initProblem = async (dir) => {
  fs.writeFileSync(path.join(dir, "Question.txt"), "");
  fs.writeFileSync(
    path.join(dir, "CodeMain.java"),
    fs.readFileSync(path.join(ASSETS_DIR, "CodeMain.java"))
  );
...
export const discardProblem = async (dir) => {
  ...
  fs.rmSync(path.join(dir, "CodeMain.java"));
